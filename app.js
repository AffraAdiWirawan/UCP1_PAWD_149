...
app.get('/', (req, res) => {
    mysql.query('SELECT * FROM items1', (err, results) => {
        if (err) throw err;
...
app.post('/add', upload.single('foto'), (req, res) => {
    ...
    mysql.query('INSERT INTO items1 (Nama, Alamat, foto, No_Telepon) VALUES (?, ?, ?, ?)', [Nama, Alamat, foto, No_Telepon], (err, result) => {
        if (err) throw err;
        res.redirect('/');
    });
});
...
app.get('/edit/:id', (req, res) => {
    ...
    mysql.query('SELECT * FROM items1 WHERE id = ?', [id], (err, results) => {
        if (err) throw err;
...
app.post('/edit/:id', upload.single('foto'), (req, res) => {
    ...
    mysql.query('UPDATE items1 SET Nama = ?, Alamat = ?, foto = ?, No_Telepon = ? WHERE id = ?', [Nama, Alamat, foto, No_Telepon, id], (err, result) => {
        if (err) throw err;
        res.redirect('/');
    });
});
...
app.get('/delete/:id', (req, res) => {
    const id = req.params.id;
    mysql.query('DELETE FROM items1 WHERE id = ?', [id], (err, result) => {
        if (err) throw err;
        res.redirect('/');
    });
});
